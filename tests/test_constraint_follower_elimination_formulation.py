...
class EliminationFormulationTest(TestCase):
    def setUp(self):
        ...
        # Assumption: Coupled dofs 0 and 1
        def g_holo(u, t):
            return np.array(u[0] - u[1], dtype=float, ndmin=1)

        def B_holo(u, t):
            return csr_matrix(np.array([[1.0, -1.0, 0]], dtype=float, ndmin=2))
        ...
        self.formulation = FollowerEliminationConstraintFormulation(self.no_of_dofs_unconstrained, self.M_func,
                                                                   self.h_func, self.B_holo_func, self.p_func,
                                                                   self.h_q_func, self.h_dq_func,
                                                                   g_func=self.g_holo_func)

        self.formulation_dense = FollowerEliminationConstraintFormulation(self.no_of_dofs_unconstrained,
                                                                         self.M_func_dense,
        ...
        self.L_desired = csr_matrix(np.array([[1, 0], [1, 0], [0, 1]], dtype=float))
    ...
    def test_recover_u_du_ddu(self):
        ...
        assert_array_equal(u, np.concatenate((np.array([x[0]], ndmin=1), x)))
        assert_array_equal(du, np.concatenate((np.array([dx[0]], ndmin=1), dx)))
        assert_array_equal(ddu, np.concatenate((np.array([ddx[0]], ndmin=1), ddx)))
    ...
    def test_M(self):
        ...
        M_desired = self.L_desired.T @ self.M_unconstr.copy() @ self.L_desired

        M_actual = self.formulation.M(x, dx, 0.0)
        M_actual_dense = self.formulation_dense.M(x, dx, 0.0)
        assert_allclose(M_actual.todense(), M_desired.todense())
        assert_allclose(M_actual_dense, M_desired.todense())
    ...
    def test_f_int(self):
        ...
        u = np.concatenate((np.array([x[0]], ndmin=1), x), axis=0)
        du = np.concatenate((np.array([dx[0]], ndmin=1), dx), axis=0)
        t = 0.0
        f_int_desired = self.L_desired.T @ self.h_func(u, du, t).copy()
    ...
    def test_f_ext(self):
        ...
        du = np.concatenate((np.array([dx[0]], ndmin=1), dx), axis=0)
        t = 0.0
        f_ext_desired = self.L_desired.T @ self.p_func(u, du, t).copy()
    ...
    def test_D(self):
        ...
        D_desired = self.L_desired.T @ self.D_unconstr.copy() @ self.L_desired
    ...
    def test_K(self):
        ...
        K_desired = self.L_desired.T @ self.K_unconstr.copy() @ self.L_desired
    ...
    def test_L_without_constraints(self):
        ...
        formulation = FollowerEliminationConstraintFormulation(self.no_of_dofs_unconstrained, self.M_func,
